...
def index():
    ...
    posts = current_user.get_followed_posts().paginate(page, current_app.config['POSTS_PER_PAGE'], False)
...
@bp.route('/explore')
@login_required
def explore(): # Page that will show all the recent posts of Microblog users
    page = request.args.get('page', 1, type=int)
...
def user_page(username):
    ...
    next_url = url_for('main.user_page', username=user.username, page=posts.next_num) if posts.has_next else None
    prev_url = url_for('main.user_page', username=user.username, page=posts.prev_num) if posts.has_prev else None
    return render_template('user_page.html', user=user, posts=posts.items, next_url=next_url, prev_url=prev_url)
...
def follow(username):
    ...
    if user == current_user:
        ...
        redirect(url_for('main.user_page', username=username))
...
def unfollow(username):
    ...
    if user is None:
        ...
        return redirect(url_for('main.index'))
    if user == current_user:
        flash(_('You cannot hide from yourself'))
        redirect(url_for('main.user_page', username=username))
...
def search():
    ...
    page = request.args.get('page', 1, type=int)
    current_app.logger.info(g.search_form.q)
